...
{
    float3 diffuseColor = diffuseTexture.sample(samplr, vert.texCoords).rgb * CAST(uniforms.material.ambientColor);

    float3 ambientTerm = { 0,0,0 }; // CAST(uniforms.light[0].ambientColor) * diffuseColor;
    
    float3 normal = vert.normal; // normalize(vert.normal);
